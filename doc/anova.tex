...
This is how the values are computed.
...
In the above table, $n$ is the number of observations and $p$ is the 
number of model parameters including the intercept term if there is one.
The sums of squares are computed as follows where
$y$ are observed values and $\hat y$ are values predicted by the model.
\begin{align*}
...
is better than the mean of all the data $\bar y$.
